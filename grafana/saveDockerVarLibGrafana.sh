...
DEST_DIR=/media/bigDisk/03-DEV/3-Docker/grafana-bind-mount/$GRAFANA_DIR
BLU='\033[34m'
VIO='\033[35m'
RED='\033[31m'
YEL='\033[33m'
...
if [ $# -eq 1 ] ; then
  ...
  echo -e "Use default destination directory: $YEL$DEST_DIR$ENDC"
...
if [ ! -d $DEST_DIR ] ; then
  echo -e "\nINFO, create destination directory:"
  mkdir -pv $DEST_DIR
  if [ ! -d $DEST_DIR ] ; then
...
echo "CMD: sudo docker cp  $ID:$GRAFANA_DIR $DEST_DIR"
sudo docker cp  $ID:$GRAFANA_DIR $DEST_DIR
